...
export default function Invaders({ onScore, onGameOver, onWin }) {
  ...
  useEffect(() => {
    ...
    let invDir=1,invX=0,invY=0,invShoot=0,cooldown=0,tick=0
    ...
    function draw(){
      ...
      ;[[22,18],[80,45],[140,22],[200,60],[250,30],[60,90],[170,80],[230,100]].forEach(([x,y])=>{
        ctx.fillRect(x,y+(tick*0.1)%CH,1,1)
      })
      // Invaders
      ...
      ctx.textAlign='right';ctx.fillText(score,275,14)
    }
    ...
    function loop(){
      if(!alive)return
      tick++
      bullets.forEach(b=>b.y-=6);invBullets.forEach(b=>b.y+=2.5)
      bullets=bullets.filter(b=>b.y>0);invBullets=invBullets.filter(b=>b.y<CH)
      invX+=invDir*0.45
      ...
      if(right>=CW-2||left<=2){invDir=-invDir;invY+=10}
      if(living.some(i=>i.y+invY+10>=CH-20)){alive=false;onGameOver(score);return}
      bullets.forEach((b,bi)=>invaders.forEach(inv=>{
        if(!inv.alive)return
        if(b.x+2>inv.x+invX&&b.x<inv.x+invX+20&&b.y+10>inv.y+invY&&b.y<inv.y+invY+12){
    ...
    function onKey(e){
      ...
      else if(e.key==='ArrowRight')player.x=Math.min(CW-player.w,player.x+15)
      else if(e.key===' '&&cooldown<=0){
        bullets.push({x:player.x+player.w/2-1,y:CH-30});cooldown=12;e.preventDefault()
      }
    }
