...
export default function Invaders({ onScore, onGameOver, onWin }) {
  ...
  useEffect(() => {
    ...
    let autoShootTimer=0  // auto attack timer
    ...
    function draw(){
      ...
      ;[[22,18],[80,45],[140,22],[200,60],[250,30],[60,90],[170,80],[230,100]].forEach(([x,y])=>{
        ctx.fillRect(x,(y+tick*0.08)%CH,1,1)
      })
      ...
      ctx.textAlign='left';ctx.fillStyle='rgba(0,255,200,0.3)'
      ctx.fillText('AUTO-FIRE ON',6,14)
    }
    ...
    function loop(){
      ...
      invBullets.forEach(b=>b.y+=2.5)
      bullets=bullets.filter(b=>b.y>0)
      invBullets=invBullets.filter(b=>b.y<CH)

      // Auto attack — fires every 18 frames
      autoShootTimer++
    ...
    function onKey(e){
      ...
      // Space still works as manual boost shot
