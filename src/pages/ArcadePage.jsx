  ...
  return (
    <>
      <style>{`
        ...
        .game-strip { display:grid; grid-template-columns:repeat(5,1fr); border-bottom:1px solid var(--border); }
        .game-strip-btn {
        ...
        .game-strip-btn:hover, .game-strip-btn.active { color:var(--accent); background:rgba(0,255,200,0.04); }
        .game-strip-btn.active::after { content:''; position:absolute; bottom:0; left:0; right:0; height:2px; background:var(--accent); box-shadow:0 0 6px var(--accent); }
        ...
        .play-left { border-right:1px solid var(--border); display:flex; flex-direction:column; align-items:center; justify-content:center; padding:28px; position:relative; gap:16px; }
        .play-right { padding:20px; overflow-y:auto; }
        .score-strip { display:flex; gap:28px; justify-content:center; }
        ...
        .go-overlay { position:absolute; inset:0; background:rgba(3,5,10,0.88); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; }
        ...
        .go-name-input { background:var(--surface2); border:1px solid var(--border); color:var(--text); font-family:var(--mono); font-size:11px; padding:6px 10px; outline:none; width:130px; transition:border-color 0.15s; }
        .go-name-input:focus { border-color:var(--accent); }
        .go-name-input::placeholder { color:var(--muted); }
        .neon-btn { font-family:var(--mono); font-size:10px; letter-spacing:0.12em; text-transform:uppercase; padding:8px 16px; border:1px solid var(--accent); background:transparent; color:var(--accent); cursor:pointer; transition:all 0.15s; border-radius:2px; }
        ...
        .panel-lbl { font-family:var(--mono); font-size:8px; letter-spacing:0.2em; text-transform:uppercase; color:rgba(0,255,200,0.35); margin-bottom:8px; padding-bottom:6px; border-bottom:1px solid var(--border); }
        .panel-section { margin-bottom:20px; }
        .hint-text { font-family:var(--mono); font-size:10px; color:var(--muted); line-height:1.8; }
        .lb-row { display:flex; align-items:center; gap:8px; padding:5px 0; border-bottom:1px solid rgba(0,255,200,0.04); font-family:var(--mono); font-size:10px; }
        ...
        .no-game { display:flex; flex-direction:column; align-items:center; justify-content:center; padding:60px; color:var(--muted); text-align:center; gap:12px; }
        ...
        .touch-controls { display:none; gap:8px; justify-content:center; flex-wrap:wrap; margin-top:8px; }
        .touch-btn { width:44px; height:44px; background:rgba(0,255,200,0.06); border:1px solid rgba(0,255,200,0.2); color:var(--accent); font-size:18px; display:flex; align-items:center; justify-content:center; cursor:pointer; border-radius:2px; -webkit-user-select:none; user-select:none; transition:background 0.1s; }
                  ...
                  {ended && (
                    <div className="go-overlay">
                      ...
                      {!submitted ? (
                        ...
                        <div style={{fontFamily:'var(--mono)',fontSize:'10px',color:'var(--accent)'}}>✓ Score saved!</div>
                      )}
                ...
                {(active.id==='tetris'||active.id==='snake'||active.id==='breakout'||active.id==='invaders') && (
                  <div className="touch-controls">
                    ...
                    <button className="touch-btn" style={{width:'80px',fontSize:'11px',fontFamily:'var(--mono)',letterSpacing:'0.1em'}}
                      onTouchStart={()=>window.dispatchEvent(new KeyboardEvent('keydown',{key:' ',bubbles:true}))}>SPC</button>
                  </div>
                )}
                {(active.id==='flappy'||active.id==='runner') && (
                  <div className="touch-controls">
                    <button className="touch-btn" style={{width:'120px',fontSize:'11px',fontFamily:'var(--mono)'}}
                      onTouchStart={()=>window.dispatchEvent(new KeyboardEvent('keydown',{key:' ',bubbles:true}))}>TAP / JUMP</button>
                  </div>
                )}
              </>
            ) : (
              ...
              <>
                ...
                <div className="panel-section">
                  ...
                  {getTop(active.id).length
                    ...
                    : <div style={{fontFamily:'var(--mono)',fontSize:'10px',color:'var(--muted)',padding:'6px 0'}}>No scores yet. Be first.</div>
                  }
                </div>
              </>
            ) : (
